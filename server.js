...
if (modo == 'CLUSTER') {
    ...
    /* WORKERS */
        ...
        worker(app, server)
        app.listen(PORT, err => {
            if (!err){ logger.log('info',`Servidor express escuchando en el puerto ${PORT} - PID WORKER ${process.pid}`)
                // console.log(`Servidor express escuchando en el puerto ${PORT} - PID WORKER ${process.pid}`)
        }
        else{logger.log('error',`No se pudo conectar al servidor en el puerto ${PORT}`)}
        })
    }
}
